    ...
    while (1) {
        ...
        if (!game_state->current_player->sleep_time) {
            ...
            if(STR_EQU(COMMAND_ROLL, com_buf)){
                ...
                Rent(game_state->current_player->cur_pos, game_state);
            ...
            else if (STR_EQU(COMMAND_DUMP, com_buf)) {
                Dump(game_state);
            }
...
//            }
            ...
            else if (STR_EQU(COMMAND_SAVE, com_buf)) {
                game_save();
            }
    ...
    for(int i = 0; i <= 10; ++i){
        if(game_state->map[pos_range+i].house_flag != NULL){
            if(game_state->map[pos_range+i].house_flag->flag == BOMB){
                ...
                game_state->map[pos_range + i].ToolType = T_NO;
            }
